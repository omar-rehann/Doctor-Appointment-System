...
const Loginaction = async () => {
  try {
    ...
    if(result.success===true){
      ...
      if (data.email === "omarrehan724@gmail.com") {
        router.push("/fetures/Admin/adddoctor");
      }else if(alldocotr.some((e) => e.email === data.email )){
        router.push("/fetures/Doctor/showdoctor");

      } else {
        // Normal User
        router.push("/fetures/User/Homepage");
      }
    }else{
       Swal.fire({
      icon: "error",
      title: "Error",
      text: "Data Invalid Try Again",
    });
    }
  ...
  <div className="flex items-center justify-between text-sm">
    <p className="text-gray-400">Create New Account ? <Link href="../../fetures/Register"><span className="cursor-pointer">Regsiter</span></Link></p>
  </div>
  <button
  onClick={Loginaction}
    type="submit"
